...
def plothrsmooth(df, size=15):
    """
        Plots smoothed hr plot (with LOWESS smoothing)
        Args:
            (pd.DataFrame): dataframe of data with DateTime, Time and Heart Rate columns
            size (integer): font size for plot (default=15)
        Returns:
            LOWESS-smoothed plot of hr
            
    """
    filteres = lowess(df['Heart Rate'], df['Time'], is_sorted=True, frac=0.025, it=0)
    filtered = pd.to_datetime(filteres[:,0], format='%Y-%m-%dT%H:%M:%S') 
    
    plt.figure(figsize=(20,5))
    plt.rcParams.update({'font.size': size})
    plt.plot(df['Time'], df['Heart Rate'], '.')
    plt.plot(filtered, filteres[:,1], 'r')
...
def plotglucosesd(df, sd=1, size=15):
    """
        Plots hr with specified standard deviation lines
        Args:
            (pd.DataFrame): dataframe of data with DateTime, Time and Heart Rate columns
            sd (integer): standard deviation lines to plot (default=1)
            size (integer): font size for plot (default=15)
        Returns:
            plot of hr with standard deviation lines
